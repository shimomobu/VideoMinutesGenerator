...
**v0.5 変更点**
- REST API レイヤー（FastAPI）を追加。外部アプリからの呼び出しを可能にした（§9.5 参照）
- `api/` ディレクトリを新設（app.py / routes.py / models.py / service.py）
...
| モジュール | パス | 責務 |
|---|---|---|
| ingest | src/ingest/ | 動画・音声ファイルの受け付けと検証（mp4/mov/mkv/wav/mp3/m4a） |
...
| 処理 | 対応形式チェック（mp4/mov/mkv/wav/mp3/m4a）、ファイル存在確認、ジョブID発行 |
| 主要関数 | `validate_input_file()`（旧 `validate_video_file` は互換エイリアスとして保持） |
| 外部依存 | なし |
| 対応形式 | VIDEO_FORMATS = {mp4, mov, mkv} / AUDIO_FORMATS = {wav, mp3, m4a} / SUPPORTED_FORMATS = VIDEO_FORMATS ∪ AUDIO_FORMATS |
| MVP範囲 | wav/mp3/m4a を既存 pipeline（FFmpeg 経由）でそのまま処理する |
| 将来課題 | wav 二重変換の最適化（入力が既に 16kHz mono の場合はスキップ）、大容量音声対応、アップロード再開 |
...
| 処理 | FFmpeg で音声・動画を 16kHz モノラル WAV に変換（形式非依存）、無音検出 |
| 外部依存 | FFmpeg（ローカル実行） |
...
| 外部依存 | Ollama native API（ローカル `http://localhost:11434`、`/api/chat` エンドポイント）。外部送信なし |
| 送信ポリシー | §5.4 参照 |
...
- analysis/extractor は Ollama native API（`http://localhost:11434/api/chat`）を通じてローカルの Gemma 4 を呼び出す
- ネットワーク接続不要。会議内容が外部サービスに送信されることはない
- Ollama が未起動の場合は `LLMError` でエラーメッセージに `base_url` / `model` / 起動確認方法を含める
- **再現性確保**: Ollama native API `/api/chat` を使用し、`options.temperature=0`（greedy decoding）と `options.seed=42` を固定する。OpenAI 互換 API（`/v1/chat/completions`）では seed の転送が不安定なため native API に切り替えた。同一 transcript に対して同一の analysis.json が生成されることを保証する
...
> **`LLMTimeoutError` はリトライ対象外（即失敗）**  
> タイムアウト発生後の Ollama 接続状態が不安定になるため、リトライせず即失敗とする設計。  
> `max_retries` 設定はタイムアウト以外の `LLMError`（接続拒否・スキーマ不正など）にのみ適用される。  
> 実装: `src/vmg/analysis/extractor/__init__.py` — `except LLMTimeoutError: raise`
...
`api/` ディレクトリとして FastAPI ベースの REST API を実装する。外部アプリからの呼び出しを可能にする。

#### エンドポイント

| メソッド | パス | 説明 |
|---|---|---|
| POST | /jobs | ファイルアップロードでジョブ開始。job_id を返す |
| GET | /jobs/{job_id} | ジョブ状態を返す（pending / running / completed / failed） |
| GET | /jobs/{job_id}/result | 結果取得（format=json / md を query param で指定可） |

#### POST /jobs リクエスト形式

```
Content-Type: multipart/form-data
- file: 動画・音声ファイル（mp4/mov/mkv/wav/mp3/m4a）
- title: 会議タイトル（文字列）
- datetime: 会議日時（ISO 8601 形式）
- participants: 参加者（カンマ区切り文字列）
```
...
| REST API | FastAPI + uvicorn | 0.115+ / 0.34+ | 非同期・型安全・OpenAPI 自動生成（`pip install ".[api]"` で追加） |
