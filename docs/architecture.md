...
4. analysis（4段階のサブ処理）
   ...
   4-2. extractor
        入力: プロンプト
        処理: Claude API を呼び出し、構造化出力（JSON）を取得する
              ※ 外部送信対象はテキストのみ（音声・映像は送信しない）
        出力: LLM生成 JSON（未検証）
...
| analysis/extractor | src/analysis/extractor/ | Claude API 呼び出し・構造化出力取得 |
...
| 処理 | Claude API を呼び出し、要約・議題・決定事項・保留事項・ToDo候補を取得する |
| 外部依存 | Anthropic Claude API（テキストのみ送信。音声・映像は送信しない） |
| 送信ポリシー | §5.4 参照 |
...
### 5.4 Claude API 外部送信ポリシー

本システムの外部API送信に関するポリシーを以下の通り定義する。

#### 送信対象

| データ種別 | 送信可否 | 備考 |
|---|---|---|
| 音声ファイル（WAV等） | **禁止** | ローカル処理のみ（Whisper ローカル実行） |
| 動画ファイル | **禁止** | ローカル処理のみ |
| transcript テキスト | **許可（デフォルト）** | analysis/extractor が Claude API に送信する |

#### transcript テキスト送信の扱い

- transcript は会議の発言内容であり、機密情報を含む可能性がある
- デフォルト構成では transcript テキストを Claude API（外部）に送信する
- 送信前に内容の機密レベルを確認することを利用者に案内する

#### 将来の拡張ポイント（外部送信ポリシーの強化）

以下のモードを将来追加できる設計とする。実装は analysis/input_builder または analysis/extractor に加える。

| モード | 内容 |
|---|---|
| 匿名化モード | 送信前に人名・組織名・固有名詞をマスクして送信する |
| ローカルLLMモード | Ollama等のローカルLLMに差し替え、外部送信ゼロにする |
| 外部送信禁止モード | Claude API 呼び出しを完全に禁止し、手動解析のみ許可する |

これらのモードは設定ファイル（`config/default.yaml`）の `api_policy` セクションで切り替えられる設計とする。
...
| LLMError | Claude API 応答失敗・スキーマ不正 | 再試行（最大N回）後に失敗扱い |
...
| 固有名詞補正 | なし（MVP外） | asr 後の後処理として辞書ベースの Corrector を追加する |
...
| 方針 | 内容 |
|---|---|
| ローカル完結 | 音声・映像データを外部に送信しない構成をデフォルトとする |
| テキストのみ外部送信 | Claude API には文字起こし結果（テキスト）のみを送信する |
...
| LLM解析 | Claude API（claude-sonnet-4-6） | - | 日本語精度・構造化出力 |
...
| LLM解析のみ外部送信 | テキスト（transcript）のみ送信 | 音声・映像は送信しないことでセキュリティリスクを最小化する |
