...
- 新機能・仕様変更・設計判断を伴う作業では、必ず実装前に設計ドキュメントへ反映する
- 対象文書は原則として以下とする
  - `docs/architecture.md`
  - `docs/task-breakdown.md`
  - 必要に応じて `README.md`
  - 必要に応じて `docs/test-plan.md`
  - 必要に応じて `docs/evaluation-plan.md`
- 実装が文書とズレる場合は、勝手に実装を優先しない
- ズレを見つけたら、差分を明示して確認を促す
- 文書修正が必要なら、実装前に最小差分で反映する
- 設計反映なしで実装に進もうとしている場合は、作業を止めて指摘する
- MVP範囲と将来課題を必ず分離して記載する
---
...
1. タスク内容が設計ドキュメント更新を必要とするか確認する
2. 新機能・仕様変更・設計判断を伴う場合は、先に設計ドキュメントを更新する
3. MVP範囲と将来課題を明確に分離する
4. 先に失敗テストを作る
5. 最小差分で実装する
6. テストを通す
7. 完了条件を確認する
...
設計ドキュメント未反映のまま実装依頼が来た場合は、実装に進まず、先に設計反映が必要であることを指摘する。
...
- 不要なリファクタや横展開はしない
- 不明点があれば、推測で広げず差分を示す
- 新機能・仕様変更・設計判断を伴う作業では、実装前に必ず設計ドキュメント反映を提案・実施する
- 設計反映がないまま実装に進む指示が来た場合は、作業を止めて「先に設計反映が必要」と指摘する
- デモ用・暫定対応・MVP外の判断は、必ず文書上に明記する