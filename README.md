...
> **注意: デモ用途のみ。本番利用を前提としない。**

```bash
# Streamlit をインストール（初回のみ）
pip install ".[ui]"
# または
...
ブラウザが自動で開く（http://localhost:8501）。動画ファイルをアップロードしてタイトル・日時・参加者を入力し、「議事録を生成する」ボタンを押すと処理が始まる。

| 制限事項 | 内容 |
|---|---|
| 処理中の表示 | `run_pipeline()` を同期実行するため、処理中（数分〜十数分）はページが固まって見える |
| 進捗バー | なし（`st.spinner` のみ） |
| キャンセル機能 | なし |
| 非同期化 | 未実装（将来課題） |
| 本番利用 | 想定していない |
